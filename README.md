...
The input files can be generated by running `versionutil` without any parameters. If you pass `--preRelease` the `preRelease` block will be included in the `version-build.json` file.
...
When `preRelease` exists, the generated version includes both the prerelease and build parts.
