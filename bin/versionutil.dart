...
void main(List<String> args) {
  ...
  if (!versionFile.existsSync()) {
    ...
    createVersionFile(versionFile,0,1);
  }
  ...
  final versionString = '$version.$revision.$formattedBuild';

  outputFile.createSync(recursive: true);
  outputFile.writeAsStringSync(_generate(lang, versionString, version, revision, build));
...
Map<String, String> _parseArgs(List<String> args) {
  ...
  for (final arg in args) {
    if (arg.startsWith('--')) {
      ...
      map[parts[0]] = parts.length > 1 ? parts[1] : 'true';
    }
  }
...
void createVersionFile(File versionFile, int version, int revision) {
  Map<String, dynamic> versionData = {"version": version, "revision": revision};
...
String _generate(String lang, String versionString, int version, int revision, int build) {
  switch (lang) {
    case 'dart':
      return '''
